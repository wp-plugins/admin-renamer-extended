...
if (count($_POST)) {
   ...
   if(empty($newName))
   {
      $message = $oldName. ' NOT changed, new name was empty!';
      $showMsg = 'block';
   }
   else
   {
      $user_query = "UPDATE $wpdb->users SET user_login = '{$newName}' WHERE {$wpdb->users}.ID = ".intval($_GET['user_id'])." LIMIT 1";
      $wpdb->get_results( $user_query );
      $message = 'Changed '.$oldName. ' into '.$newName;
      $showMsg = 'block';
   }
}
